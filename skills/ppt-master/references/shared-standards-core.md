...
| Minimum font size floor (≥ 11 pt / 14.6 px) | **Hard rule — Minimum Font Size**: All authored text elements across the deck (including body copy, bullet items, table cells, metadata, footers, badges, and captions) MUST enforce a strict lower bound of `font-size ≥ 11 pt` (or `14.6 px` on standard 96 DPI canvas). Sub-11pt text is strictly prohibited to guarantee readability across conference projectors, external monitors, and mobile/laptop viewing. |
| Zero text overflow & bounding box fit | **Hard rule — Zero Text Overflow**: Text bounding boxes (`data-pptx-bounds`, cards, containers) MUST have strict internal margins and sufficient vertical/horizontal capacity. Text MUST NOT clip, collide with card borders, or overflow outside visible containers. Authors MUST budget line count, word-wrap, and container dimensions to ensure comfortable breathing room with zero text overflow. |
| Light theme default standard | **Hard rule — Light Theme Baseline**: Unless an explicit dark theme is locked by brand/template or explicitly requested by the user, decks MUST default to a clean, modern, high-contrast **Light Theme**: light canvas (`#F8FAFC` / `#FFFFFF`), pure white cards (`#FFFFFF`) with crisp subtle borders (`#CBD5E1` / `#E2E8F0`), high-contrast dark slate headers/text (`#0F172A` / `#334155`), and vibrant semantic accents (`#0284C7`, `#059669`, `#D97706`). |
| Contrast and semantic encoding | Within the active profile's fidelity boundary, keep meaning-bearing text distinguishable from its actual background (minimum 4.5:1 contrast for body text, 3.0:1 for large headers). For newly authored distinctions, combine luminance, weight, scale, shape, position, or explicit labeling; color may reinforce meaning but never carry a required distinction alone. Reserve lower-contrast treatment for genuinely secondary metadata that remains legible. |
| Natural wrapping | Break at semantic phrase or punctuation boundaries where possible. Reflow the text frame or adjust neighboring geometry before using any permitted local size reduction (which must never drop below 11 pt). Let the final line run naturally shorter; avoid mechanically equal lines or a stranded single-character / single-word line when an earlier natural break preserves meaning. |
