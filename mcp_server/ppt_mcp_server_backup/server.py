...
app = MCPServer(
    name="ppt-master",
    version="4.3.0",
    description="AI presentation engine for generating editable DrawingML PPTX decks"
)
...
def ppt_convert_to_pptx(project_path: str, native_charts_tables: bool = False, transition: str = "fade") -> str:
    ...
    """
    args = [project_path, "--transition", transition]
    if native_charts_tables:
...
def ppt_enhance_native_pptx(pptx_path: str, project_name: str) -> str:
    ...
    """
    res = run_script("native_enhance_pptx.py", ["init", pptx_path, "--name", project_name])
    if res["success"]:
        return f"[ENHANCE INIT SUCCESS]\n{res['stdout']}"
    else:
        return f"[ENHANCE ERROR]\n{res['stderr'] or res['stdout']}"

# -----------------------------------------------------------------------------
# MCP Resources
# -----------------------------------------------------------------------------

@app.resource("ppt://projects/{project_name}/design_spec")
...
@app.resource("ppt://projects/{project_name}/spec_lock")
